...
function filterPluginPyazoAutoInlineDisplay(message) {
...
function filterPluginGistInline(message){
...
function filterPluginAsamashiate(message) {
...
function filterPluginTwitterInline(message){
...
function filterPluginYoutubeThumbnail(message){
    // YT thumbnail
...
function filterPluginAutoLink(message){
    //auto link
    return message.replace(/(http(s)?:\/\/[\x21-\x7e]+)/gi, "<a href='$1' target='_blank'>$1</a>");
}

function filterPluginSyntaxHilight(message) {
