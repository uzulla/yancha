...
$posts = $storage->search_post( {}, { order_by => ['plusplus ASC', 'text ASC'] });
my $i = 0;
for my$msg( sort { $plusplus_values{$a} <=> $plusplus_values{$b} || $a cmp $b } keys %plusplus_values ) {
    is( $msg, $posts->[$i++]->{text} );
}

$posts = $storage->search_post( {}, { order_by => ['plusplus DESC', 'text DESC'] });
$i = 0;
for my$msg( sort { $plusplus_values{$b} <=> $plusplus_values{$a} || $b cmp $a} keys %plusplus_values ) {
    is( $msg, $posts->[$i++]->{text} );
}

$posts = $storage->search_post( {}, { order_by => ['plusplus DESC', 'text ASC'] });
$i = 0;
for my$msg( sort { $plusplus_values{$b} <=> $plusplus_values{$a} || $a cmp $b} keys %plusplus_values ) {
    is( $msg, $posts->[$i++]->{text} );
}

$posts = $storage->search_post( {}, { order_by => ['plusplus ASC', 'text DESC'] });
$i = 0;
for my$msg( sort { $plusplus_values{$a} <=> $plusplus_values{$b} || $b cmp $a} keys %plusplus_values ) {
    is( $msg, $posts->[$i++]->{text} );
}
...
test_pocketio $server => sub {
    ...
    $posts = $search_by_api->( { order => 'plusplus,text' } );
    my $i = 0;
    for my$msg( sort { $plusplus_values{$a} <=> $plusplus_values{$b} || $a cmp $b } keys %plusplus_values ) {
        is( $msg, $posts->[$i++]->{text} );
    }
