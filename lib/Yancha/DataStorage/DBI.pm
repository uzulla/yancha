...
sub init {
    my ($self) = @_;
    my ($name) = (ref $self) =~ /([^:]+)$/;
    $self->{sql_maker} = SQL::Maker->new(driver => $name);
}
...
sub _add_user {
    ...
    my ($sql, @binds) = $self->{sql_maker}->insert('user', [
        (map { $_ => $user->{$_} }
             qw/user_key nickname profile_image_url profile_url sns_data_cache/),
...
sub replace_user {
    ...
    my ($sql, @binds) = $self->{sql_maker}->update('user', [
        (map { $_ => $user->{$_} }
             qw/nickname profile_image_url profile_url sns_data_cache/),
        updated_at => $self->time_literal($now),
    ], {
...
sub add_session {
    ...
    my ($sql, @binds) = $self->{sql_maker}->insert('session', [
        user_key => $userkey,
        token => $token,
        expire_at => $self->time_literal(time + $exp),
    ]);
    return $self->dbh->do($sql, {}, @binds);
}
...
sub get_session_by_token {
    ...
    my ($sql, @binds) = $self->{sql_maker}->select('session', ['*'], {
        token => $token,
        expire_at => {'>' => $self->time_literal(time)},
    });
    return $self->dbh->selectrow_hashref($sql, {}, @binds);
}
